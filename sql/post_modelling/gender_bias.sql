...
select
	*,
	SUM(CASE WHEN gender = 'male' THEN 1 ELSE 0 END) OVER (order by k rows between unbounded preceding and current row) as cum_sum_male,
	SUM(CASE WHEN gender = 'male' THEN 1 ELSE 0 END) OVER (order by k rows between unbounded preceding and current row) as cum_sum_female
	...
	SUM(CASE WHEN label = 1 then 1 else 0 end) OVER (order by k rows between unbounded preceding and current row)::float /
